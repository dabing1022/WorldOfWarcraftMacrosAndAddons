...
CS=RF(FR) CS.c=RF("Cooldown","CST",CS.t, "CooldownFrameTemplate") CS:RegisterEvent(USD)
...
SP=RF(FR) SP.c=RF("Cooldown","SAP",SP.t, "CooldownFrameTemplate") SP:RegisterEvent(USD)
...
GR=RF(FR) GR.c=RF("Cooldown","GARROTE",GR.t, "CooldownFrameTemplate") GR:RegisterEvent(USD)
...
hooksecurefunc(UFP,function(self)
        if self.portrait then
                if UnitIsPlayer(self.unit) then				
					SetPortraitToTexture("PlayerPortrait", "Interface\\AddOns\\MinyonCF\\Coolface")				
					if self.unit == "player" or self.unit == "pet" or self.unit == "partypet1" or self.unit == "partypet2" or self.unit == "partypet3" or self.unit == "partypet4" then 
						return 
					end				
